...
public class TlsUtils {
    ...
    private static void addCertSigAlgOID(Hashtable h, ASN1ObjectIdentifier oid, short hashAlgorithm,
    ...
    private static Hashtable createCertSigAlgOIDs() {
        ...
        addCertSigAlgOID(h, PKCSObjectIdentifiers.sha224WithRSAEncryption, HashAlgorithm.sha224,
                SignatureAlgorithm.rsa);
        addCertSigAlgOID(h, PKCSObjectIdentifiers.sha256WithRSAEncryption, HashAlgorithm.sha256,
                SignatureAlgorithm.rsa);
        addCertSigAlgOID(h, PKCSObjectIdentifiers.sha384WithRSAEncryption, HashAlgorithm.sha384,
                SignatureAlgorithm.rsa);
        addCertSigAlgOID(h, PKCSObjectIdentifiers.sha512WithRSAEncryption, HashAlgorithm.sha512,
                SignatureAlgorithm.rsa);
        ...
        // TODO[RFC 8998]
        // addCertSigAlgOID(h, GMObjectIdentifiers.sm2sign_with_sm3, HashAlgorithm.sm3,
    ...
    public static short getLegacySignatureAlgorithmServer(int keyExchangeAlgorithm) {
        switch (keyExchangeAlgorithm) {
            ...
            /*
             * GB/T 38636-2020 (TLCP) - SM2 uses ECDSA signature algorithm
             */
    ...
    /**
     * @deprecated Use
     *             {@link #PRF(SecurityParameters, TlsSecret, String, byte[], int)}
     *             instead.
     */
    @Deprecated
    @SuppressWarnings("InlineMeSuggester")
        ...
        for (int i = 0; i < keyExchangeAlgorithms.size(); ++i) {
            ...
            switch (keyExchangeAlgorithm) {
                ...
                case KeyExchangeAlgorithm.NULL: {
                    // TODO[tls13] We're conservatively adding both here, though maybe only one is
                    // needed
                    addToSet(result, NamedGroupRole.dh);
                    addToSet(result, NamedGroupRole.ecdh);
    ...
    /**
     * @return Whether a server can select the specified cipher suite given the
     *         available signature
    ...
    /**
     * @deprecated Use {@link #isValidVersionForCipherSuite(int, ProtocolVersion)}
     *             instead.
     */
    @Deprecated
    @SuppressWarnings("InlineMeSuggester")
    ...
    public static boolean isValidVersionForCipherSuite(int cipherSuite, ProtocolVersion version) {
        ...
        // Special handling for TLCP cipher suites: if the minimum version is TLCP and
        // the
    ...
    public static SignatureAndHashAlgorithm chooseSignatureAndHashAlgorithm(ProtocolVersion negotiatedVersion,
        ...
        if (sigHashAlgs == null) {
            /*
             * TODO[tls13] RFC 8446 4.2.3 Clients which desire the server to authenticate
             * itself via
    ...
    /**
     * @deprecated Use {@link #getSupportedCipherSuites(TlsCrypto, int[], int, int)}
     *             instead.
     */
    @Deprecated
    @SuppressWarnings("InlineMeSuggester")
    ...
    /**
     * Check the signature algorithm for certificates in the peer's CertPath as
     * specified in RFC
     * 5246 7.4.2, 7.4.4, 7.4.6 and similar rules for earlier TLS versions. The
     * supplied CertPath
     * should include the trust anchor (its signature algorithm isn't checked, but
     * in the general
     * case checking a certificate requires the issuer certificate).
     *
     * @throws IOException
     *                     if any certificate in the CertPath (excepting the trust
     *                     anchor) has a signature
     *                     algorithm that is not one of the locally supported
    ...
    private static void checkSigAlgOfServerCerts(TlsContext context, TlsCertificate[] serverCertPath)
        ...
        /*
         * NOTE: For TLS 1.2, we'll check 'signature_algorithms' too (if it's distinct),
         * since
         * there's no way of knowing whether the server understood
        ...
        for (int i = 0; i < trustAnchorPos; ++i) {
            ...
            if (null == sigAndHashAlg) {
                ...
                /*
                 * RFC 5246 7.4.2. If the client provided a "signature_algorithms" extension,
                 * then
    ...
    static void checkTlsFeatures(Certificate serverCertificate, Hashtable clientExtensions, Hashtable serverExtensions)
            throws IOException {
        /*
         * RFC 7633 4.3.3. A client MUST treat a certificate with a TLS feature
         * extension as an
         * invalid certificate if the features offered by the server do not contain all
         * features
         * present in both the client's ClientHello message and the TLS feature
         * extension.
         */
    ...
    static void processClientCertificate(TlsServerContext serverContext, Certificate clientCertificate,
        ...
        if (isTLSv13) {
            ...
            /*
             * NOTE: We tolerate SSLv3 clients sending an empty chain, although "If no
             * suitable
             * certificate is available, the client should send a no_certificate alert
             * instead".
             */
         ...
         * at its
         * discretion either continue the handshake without client authentication, or
         * respond with a
         * fatal handshake_failure alert. Also, if some aspect of the certificate chain
         * was
         * unacceptable (e.g., it was not signed by a known, trusted CA), the server MAY
         * at its
         * discretion either continue the handshake (considering the client
         * unauthenticated) or send
         * a fatal alert.
         */
    ...
    static void processServerCertificate(TlsClientContext clientContext,
        ...
        clientAuthentication
                .notifyServerCertificate(new TlsServerCertificateImpl(serverCertificate, serverCertificateStatus));
    }
    ...
    static CertificateRequest validateCertificateRequest(CertificateRequest certificateRequest,
    ...
    static CertificateRequest normalizeCertificateRequest(CertificateRequest certificateRequest,
    ...
    static Hashtable addKeyShareToClientHello(TlsClientContext clientContext, TlsClient client,
        ...
        /*
         * RFC 8446 9.2. If containing a "supported_groups" extension, it MUST also
         * contain a
         * "key_share" extension, and vice versa. An empty KeyShare.client_shares vector
         * is
         * permitted.
         */
        ...
        // TODO[tls13-psk] When clientShares empty, consider not adding extension if
        // pre_shared_key in use
    ...
    static Hashtable addKeyShareToClientHelloRetry(TlsClientContext clientContext, Hashtable clientExtensions,
        ...
        if (clientAgreements.isEmpty() || clientShares.isEmpty()) {
            // NOTE: Probable cause is declaring an unsupported NamedGroup in
    ...
    static void negotiatedCipherSuite(SecurityParameters securityParameters, int cipherSuite) throws IOException {
        ...
        /*
         * TODO[tls13] We're slowly moving towards negotiating cipherSuite THEN version.
         * We could
         * move this to "after parameter negotiation" i.e. after
        ...
        if (isTLSv13(negotiatedVersion)) {
            ...
            /*
             * RFC 9189 4.2.6. The verify_data_length value is equal to 32 for the CTR_OMAC
             * cipher
    ...
    static void negotiatedVersionTLSClient(TlsClientContext clientContext, TlsClient client) throws IOException {
        ...
        if (!ProtocolVersion.isSupportedTLSVersionClient(negotiatedVersion)
                && !ProtocolVersion.isSupportedTLCPVersionClient(negotiatedVersion)) {
            throw new TlsFatalAlert(AlertDescription.internal_error);
        }
    ...
    static void negotiatedVersionTLSServer(TlsServerContext serverContext) throws IOException {
        ...
        if (!ProtocolVersion.isSupportedTLSVersionServer(negotiatedVersion)
                && !ProtocolVersion.isSupportedTLCPVersionServer(negotiatedVersion)) {
            throw new TlsFatalAlert(AlertDescription.internal_error);
        }
    ...
    static void checkExtensionData13(Hashtable extensions, int handshakeType, short alertDescription)
            throws IOException {
        ...
        while (e.hasMoreElements()) {
            ...
            if (null == extensionType || !isPermittedExtensionType13(handshakeType, extensionType.intValue())) {
                throw new TlsFatalAlert(alertDescription,
                        "Invalid extension: " + ExtensionType.getText(extensionType.intValue()));
            }
        }
    }
    ...
    static OfferedPsks.BindersConfig addPreSharedKeyToClientHelloRetry(TlsClientContext clientContext,
        ...
        // NOTE: psk_key_exchange_modes should already be in 'clientExtensions' from the
        // ClientHello
        ...
        if (null != offeredPsks) {
            ...
            if (PskKeyExchangeMode.psk_dhe_ke == selectedMode) {
                ...
                if (null != psk) {
                    ...
                    if (index >= 0) {
                        /*
                         * RFC 8446 4.2.11. Prior to accepting PSK key establishment, the server MUST
                         * validate the
                         * corresponding binder value [..]. If this value is not present or does not
                         * validate, the
                         * server MUST abort the handshake. Servers SHOULD NOT attempt to validate
                         * multiple
                         * binders; rather, they SHOULD select a single PSK and validate solely the
                         * binder that
